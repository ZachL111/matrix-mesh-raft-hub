...
`matrix-mesh-raft-hub` is centered on distributed systems work in SQL. The technical target is: Implement an SQL distributed systems project for raft diagnostic reporting, using negative fixtures and human-readable error snapshots.
...
The score is deterministic. The current threshold is 154. Risk has a penalty of 7, latency has a penalty of 3, and weight contributes a bonus of 5.
